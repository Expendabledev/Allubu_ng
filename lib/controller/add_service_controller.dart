...
class AddServiceController extends GetxController {
  ...
  RxList<SelectedServiceModel> selectedServiceModel =
      <SelectedServiceModel>[].obs;
  RxList<Map<String, List<OptionModel>>> services =
      <Map<String, List<OptionModel>>>[].obs;

  Future<void> getArgument() async {
    dynamic argumentData = Get.arguments;
    if (argumentData != null) {
      businessModel.value = argumentData['businessModel'];
      services.value = BusinessModel()
          .decodeFromFirebase(businessModel.value.services ?? []);
  ...
  void toggleOption(
      ServiceModel serviceModel, OptionModel option, bool selected) {
    int indexOfModel =
        selectedServiceModel.indexWhere((e) => e.id == serviceModel.id);

    if (selected) {
      if (indexOfModel == -1) {
        selectedServiceModel
            .add(SelectedServiceModel(id: serviceModel.id, options: [option]));
      } else {
        List<OptionModel> list =
            selectedServiceModel[indexOfModel].options ?? [];
  ...
  Future<void> saveDetails() async {
    ...
    for (var element in selectedServiceModel) {
      if (element.options!.isNotEmpty) {
        if (services
            .where((p0) => p0.keys.toString() == element.id.toString())
            .isEmpty) {
